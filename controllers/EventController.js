...
exports.joinEvent = async (req, res) => {
  ...
  try {
    ...
    if (event.createdBy.toString() === userId.toString()) {
      return res.status(400).json({
        message: "Vous êtes déjà participant (créateur de l'événement).",
      });
    }
    ...
    if (
      event.blacklist.some(
        (entry) => entry.user.toString() === userId.toString()
      )
    ) {
      return res
        .status(403)
        .json({ message: "Vous êtes bloqué pour participer à cet événement." });
    }
  ...
  try {
    ...
    if (event.createdBy.toString() === userId.toString()) {
      ...
      if (event.startDate <= now) {
        return res.status(400).json({
          message:
            "Vous ne pouvez pas quitter l'événement car il a déjà commencé.",
        });
      }
      ...
      if (
        !newOwnerId ||
        !event.participants.includes(newOwnerId) ||
        newOwnerId === userId
      ) {
        return res.status(400).json({
          message:
            "Vous devez choisir un participant valide pour transférer l'ownership.",
        });
      }
...
exports.updateEvent = async (req, res) => {
  ...
  try {
    ...
    if (!updatedEvent) {
      return res
        .status(404)
        .json({ message: "Évènement non trouvé lors de la mise à jour." });
    }
...
exports.deleteEvent = async (req, res) => {
  ...
  try {
    ...
    if (new Date() >= new Date(event.startDate)) {
      return res.status(400).json({
        message:
          "Vous ne pouvez pas supprimer un événement qui a déjà commencé.",
      });
    }
...
exports.excludeParticipant = async (req, res) => {
  ...
  try {
    ...
    if (event.createdBy.toString() === userId.toString()) {
      return res.status(400).json({
        message: "Vous ne pouvez pas vous exclure vous-même de l'événement.",
      });
    }
    ...
    if (!event.participants.includes(userId)) {
      return res.status(400).json({
        message: "L'utilisateur n'est pas un participant de cet événement.",
      });
    }
...
exports.getEventsByUser = async (req, res) => {
  ...
  try {
    ...
    if (events.length === 0) {
      return res
        .status(404)
        .json({ message: "Aucun événement trouvé pour cet utilisateur." });
    }
    res.json(events);
  } catch (error) {
    console.error(error.message);
...
exports.unblockParticipant = async (req, res) => {
  ...
  try {
    ...
    const isBlocked = event.blacklist.some(
      (item) => item.toString() === userId
    );
    if (!isBlocked) {
      return res.status(400).json({
        message:
          "L'utilisateur n'est pas dans la liste noire de cet événement.",
      });
    }
